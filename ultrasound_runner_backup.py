...
class UltrasoundDDRMRunner(Diffusion):
    ...
    def _enhance_tissue_pixels(self, restored_image, tissue_mask, original_image, version):
        """
        조직 영역의 픽셀값을 원본에 자연스럽게 가까워지도록 향상시키는 후처리 함수
        
        Args:
            restored_image: 복원된 이미지 (torch.Tensor)
            tissue_mask: 조직 마스크 (numpy.array)
            original_image: 원본 이미지 (torch.Tensor)
            version: 이미지 버전 (V3~V7)
        
        Returns:
            torch.Tensor: 조직 영역이 향상된 이미지
        """
        ...
        # 선별적 부드러운 경계 형성: 비조직 영역에만 적용
        try:
            from scipy import ndimage
            
            # 조직 영역을 제외한 나머지 영역에만 부드러운 처리 적용
            non_tissue_mask = tissue_mask <= 0.1  # 조직이 아닌 영역
            
            if np.any(non_tissue_mask):
                # 비조직 영역에만 가벼운 Gaussian blur 적용
                blurred_region = ndimage.gaussian_filter(enhanced_np, sigma=1.5)
                
                # 비조직 영역에만 블러 적용 (조직 경계 선명도 보존)
                enhanced_np[non_tissue_mask] = blurred_region[non_tissue_mask]
                
                non_tissue_count = np.sum(non_tissue_mask)
                logger.info(f"{version} 선별적 부드러운 처리: 비조직 영역 {non_tissue_count}개 픽셀에만 적용")
            else:
                logger.info(f"{version} 비조직 영역 없음 - 부드러운 처리 건너뛰")
                
        except ImportError:
            logger.warning("scipy 없음 - 부드러운 처리 건너뛰")
            
        logger.info(f"{version} 조직 경계 선명도 보존 + 비조직 영역 부드러운 처리 완료")
